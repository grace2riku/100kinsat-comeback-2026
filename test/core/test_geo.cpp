...
TEST_CASE("distanceMeters: 子午線方向の1度は約111.2km") {
  // (0,0) -> (1,0): 緯度1度ぶん北
  CHECK(geo::distanceMeters(0.0, 0.0, 1.0, 0.0)
        == doctest::Approx(kOneDegreeMeters).epsilon(1e-5));
}

TEST_CASE("distanceMeters: 赤道方向の1度は約111.2km") {
  // (0,0) -> (0,1): 経度1度ぶん東（赤道上）
  CHECK(geo::distanceMeters(0.0, 0.0, 0.0, 1.0)
        == doctest::Approx(kOneDegreeMeters).epsilon(1e-5));
}

TEST_CASE("distanceMeters: 同一点の距離は0") {
  CHECK(geo::distanceMeters(35.681236, 139.767125,
...
TEST_CASE("bearingDegrees: 同一点は0度（退化ケース）") {
  CHECK(geo::bearingDegrees(35.681236, 139.767125,
                            35.681236, 139.767125)
        == doctest::Approx(0.0));
}
