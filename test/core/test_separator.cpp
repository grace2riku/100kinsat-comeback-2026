...
TEST_CASE("非有限 dt(NaN) は上限到達扱いで安全側停止（HIGH に凍結しない）") {
  MockGpio g;
  separator::ParachuteSeparator sep(g, kPin, {1000.0, 1});
  sep.begin();
  REQUIRE(sep.start());
  sep.update(std::numeric_limits<double>::quiet_NaN());
  CHECK(isLow(g));  // NaN で Heating 凍結せず、必ず LOW へ倒れる
  CHECK_FALSE(sep.isHeating());
}

TEST_CASE("非有限 dt(+Inf/-Inf) も安全側停止") {
  ...
  }
  {
    ...
    sep.begin();
    REQUIRE(sep.start());
    sep.update(-kInf);  // dtMs>0.0 が false になり従来は凍結していたケース
    CHECK(isLow(g));
    CHECK_FALSE(sep.isHeating());
  }
}

TEST_CASE("非有限 dt は再試行を温存（maxAttempts>1 なら Idle に戻る）") {
  MockGpio g;
  ...
  sep.update(std::numeric_limits<double>::quiet_NaN());
  CHECK(isLow(g));
  CHECK_FALSE(sep.isDone());  // 端末化せず（上限到達扱い＝通常完了と同じ）
  CHECK(sep.start());         // もう 1 回撃てる
  CHECK(isHigh(g));
}
