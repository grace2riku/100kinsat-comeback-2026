...
| ミッション系 | モータ、電熱線、各種センサ (9 軸 / GPS / 照度)、カメラボード (赤コーン終端誘導) |
...
| カメラ | 専用コネクタ (CIF, メインボード裏面) | Spresense カメラボード (フレキシブルケーブル接続) |
...
### カメラボード (CXD5602PWBCAM1) — 専用コネクタ（Issue #52 赤コーン終端誘導）

- 接続: メインボードのカメラ専用コネクタにフレキシブルケーブルで接続（GPIO を消費しないため
  上表の既存ピンアサインと競合しない）。電源もコネクタ経由で供給される。
- センサ: ISX012 (1/4型 CMOS, 511万画素)。レンズ画角 78°±3°（対角）・EFL 2.74mm・F2.0。
  水平画角は約 66°（机上値。方位角換算の校正は `doc/development/camera_bringup.md` 手順E）。
- 搭載: 3D プリント部品 `camera-mount-20240219.stl`（§4）で機体前方へ固定する。
  終端誘導に使うため、走行姿勢で光軸が進行方向正面を向くように取り付ける。
- 用途: 種目5 の目標物「赤色無地カラーコーン（高さ70cm・幅37.5cm）」の画像検出
  （ソフトウェア仕様は `software.md` §5.10）。
