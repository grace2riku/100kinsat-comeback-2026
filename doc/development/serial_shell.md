...
| `imu` | `imu [init\|cal\|mon [n]]` | 9軸センサ(BNO055)の方位・校正を読む（init=再初期化 / cal=校正のみ / mon=連続表示） | 実動作（Issue #9） |
...
> `motor`（Issue #8）・`imu`（Issue #9）は実装済み。Phase2 では残る `gnss`/`log` のスタブを、各 hal モジュール（Issue #10/#14）の呼び出しに置き換える。
