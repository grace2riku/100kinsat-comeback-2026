...
| `cds` | `cds [mon [n]]` | 照度センサ(A0)の生値/放出検知（無印=生値の単発 / mon=連続観測で暗→明の継続→放出確定） | 実動作（Issue #11） |
| `beep` | `beep <freq_hz> <ms>` | スピーカ(D09)を指定周波数・時間で鳴らす | 実動作 |
| `notify` | `notify <boot\|goal\|error> [sec] \| stop` | 状態通知パターン再生（スピーカD09+LED。boot=起動音1回 / goal=ゴール通知 / error=エラー通知 / stop=消音消灯） | 実動作（Issue #15） |
| `motor` | `motor <forward\|back\|left\|right\|stop> [duty 0-255] [ms]` | モータ駆動(TB6612FNG)。指定方向へ駆動し ms 後に自動停止 | 実動作（Issue #8） |
| `imu` | `imu [init\|stat\|cal\|mon [n]\|i2cdiag [n]]` | 9軸センサ(BNO055)の方位・校正・状態を読む（init=再初期化 / stat=健全性 / cal=校正のみ / mon=連続表示） | 実動作（Issue #9） |
...
| `cam` | `cam <init\|snap\|dump\|detect\|mon [n]\|thr [yMin yMax uMax vMin]>` | カメラ撮像・赤コーン検出（init=初期化(WB=DAYLIGHT固定) / snap=VGA JPEGをSD保存 / dump=QVGA YUV422(UYVY)生データをSD保存 / detect=単発検出+処理時間 / mon=連続検出 / thr=色閾値の表示・設定） | 実動作（Issue #52） |
...
> `motor`（#8）・`imu`（#9）・`gnss`（#10）・`cds` 放出検知（#11）・`land`（#12）・`separate`（#13）・`log`（#14）・`notify`（#15）・`cam`（#52）は実装済み。残る Phase2 単体機能は TWELITE テレメトリ（#16）。
