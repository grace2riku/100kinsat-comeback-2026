...
## 4. clangd 用の補完設定（Arduino.h 等の誤検知解消）
...
> スクリプトは内部で `arduino-cli compile --build-path <tmp>` を実行し、生成された `compile_commands.json` をコピーしている。
...
| clangd が `Arduino.h` を見つけられない | 手順4で `compile_commands.json` を生成・配置 |
